...
const instagramQueryId = "e769aa130647d2354c40ea6a439bfc08"
...
func main() {
	...
	c.OnResponse(func(r *colly.Response) {
		if strings.Index(r.Headers.Get("Content-Type"), "image") > -1 {
			log.Println("save >>> ")
			r.Save(outputDir + r.FileName() + ".jpg")
			return
		}

		if strings.Index(r.Headers.Get("Content-Type"), "video") > -1 {
			log.Println("save >>> ")
			r.Save(outputDir + r.FileName() + ".mp4")
			return
		}
		...
		for _, obj := range data.Data.User.Container.Edges {
			...
			if obj.Node.IsVideo {
				log.Println("skip videos >>>")
				c.Visit(obj.Node.VideoURL) //尝试下载
			}
			c.Visit(obj.Node.ImageURL)
		}
